...
#
device_id = (
    None  # "923322071108" # serial number of device to use or None to use default
)
enable_imu = True
enable_rgb = True
enable_depth = True
    ...
    while True:
        ...
        if enable_rgb or enable_depth:
            frames = pipeline.wait_for_frames(
                200 if (frame_count > 1) else 10000
            )  # wait 10 seconds for first frame
        ...
        if enable_rgb or enable_depth:
            ...
            aligned_frames = (
                align.process(frames) if enable_depth and enable_rgb else None
            )
            depth_frame = (
                aligned_frames.get_depth_frame()
                if aligned_frames is not None
                else frames.get_depth_frame()
            )
            color_frame = (
            ...
            depth_image = (
                np.asanyarray(depth_frame.get_data()) if enable_depth else None
            )
            ...
            # Verify streams by printing shapes
            if enable_depth and depth_image is not None:
                print(
                    f"Depth frame shape: {depth_image.shape}, dtype: {depth_image.dtype}"
                )
            if enable_rgb and color_image is not None:
                print(
                    f"RGB frame shape: {color_image.shape}, dtype: {color_image.dtype}"
                )

            # Optional: Save one frame for verification
            if frame_count == 1 and enable_rgb:
                cv2.imwrite("rgb_frame.png", color_image)
                print("Saved RGB frame to rgb_frame.png")

        if enable_imu:
            accel_frame = imu_frames.first_or_default(
                rs.stream.accel, rs.format.motion_xyz32f
            )
            gyro_frame = imu_frames.first_or_default(
